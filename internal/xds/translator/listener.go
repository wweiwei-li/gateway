...
func (t *Translator) addHCMToXDSListener(
	...
	if irListener.TLS != nil {
		...
		if len(irListener.Hostnames) > 0 {
			hostname = irListener.Hostnames[0]
		}
		certResolutions, err := t.resolveExtensionCertificates(irListener.TLS, irListener.Name, irListener.Port, hostname)
		if err != nil {
			return err
		}
...
func (t *Translator) addXdsTCPFilterChain(
	...
	if isTLSTerminate {
		certResolutions, err := t.resolveExtensionCertificates(irRoute.TLS.Terminate, irRoute.Name, 0, "")
		if err != nil {
			return err
		}
...
func buildXdsTLSCertSecret(tlsConfig *ir.TLSCertificate) *tlsv3.Secret {
	...
	// An extension-resolved certificate carries no key material: the extension server decides
	// how Envoy obtains it, and may return a Secret of its own. Emitting one here would put an
	// empty chain and key into the xDS stream.
...
// results keyed by IR certificate name.
//
// Admission has already confirmed the reference is permitted and that the resource reports
// Ready, so a failure here is a translation failure rather than a pending state.
func (t *Translator) resolveExtensionCertificates(
	...
	for i := range tlsConfig.Certificates {
		...
		// The IR listener name carries namespace, gateway and listener as "ns/gateway/listener".
		gatewayNamespace, gatewayName, listenerName := splitIRListenerName(irName)

		resolution, err := hookClient.PostTLSCertificateResolveHook(&extensionTypes.TLSCertificateContext{
			Certificate:      cert.ExtensionRef.Object,
		...
		if resolution == nil {
			return nil, fmt.Errorf("extension returned no resolution for certificate %q", cert.Name)
		}
		// A refusal concerns one certificate, so it must not fail translation for the whole
		// Gateway -- that would take unrelated listeners down while a single certificate is
		// still propagating. Leaving it unresolved omits it from the filter chain, so only the
		// listener that referenced it stops serving.
		...
		// Clusters and inline secrets would have to be added to the resource table, which is
		// not reachable from the listener build path. Rejecting them is preferable to
		// accepting a config whose dependencies were silently dropped.
		if len(resolution.Clusters) > 0 {
			return nil, fmt.Errorf("certificate %q: extension returned clusters, which are not yet supported", cert.Name)
		}
		if len(resolution.Secrets) > 0 {
			return nil, fmt.Errorf("certificate %q: extension returned secrets, which are not yet supported", cert.Name)
		}
...
// splitIRListenerName decomposes an IR listener name of the form "namespace/gateway/listener".
// Names that do not follow that shape yield empty components rather than an error, since this
// only populates advisory context for an extension server.
